...
const statusText = computed(() => {
  ...
  if (allQuakes.value.length > 0)
    return `✅ 共 ${allQuakes.value.length} 筆地震資料（顯著 + 小區域）` // API Key 已硬編碼，無需提示設定
  return '' // 如果沒有地震資料且非載入中/錯誤，則不顯示狀態文字，由 .empty 處理
})
...
onMounted(() => {
  initMap()
  // 直接設定 API Key 並載入資料
  apiKey.value = 'CWA-612B7BDA-FE3C-4429-A3C4-9981A8A4EC65' // 硬編碼的 API Key
  load()
})
